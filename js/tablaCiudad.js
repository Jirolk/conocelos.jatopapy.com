$(document).ready(function() {
    $('#tablaCiudades').DataTable({
        ...
        buttons: [
            ...
            // },
            {
                text: "<i class='fa fa-plus' aria-hidden='true' > <b>Nueva Ciudad</b> </i>",
                attr: {
                    class: "btn bt mb-3",
                    style: "color: white;"
                },
...
function registrar() {
    ...
    $.ajax({
        ...
        if (resp == 1) {
            alertify.error("La ciudad ingresada ya existe. Cambie por otra");
            $("#ciudadN").focus();
        } else if (resp == 2) {
            alertify.alert("Atención", "El registro de la Ciudad fue Registrado con éxito!",
                function() {
                    location.reload();
                }
            );
...
function validarCampos() {
    if ($("#accion").val() == "N") {
        if ($("#ciudadN").val().length < 3) {
            alertify.error("Ingrese como mínimo 3 dígitos en Ciudad");
            $("#ciudadN").focus();
        } else {
            registrar();
        }
...
function eliminar(id) {
    $.ajax({
        ...
        if (resp == 6) {
            ...
            alertify.alert("Atención", "Error al Eliminar, la Ciudad ya esta relacionada con otra tabla!",
                function() {
                    location.reload();
                }
            );
...
function actualizar() {
    ...
    $.ajax({
        ...
        if (resp == 3) {
            alertify.error("La Ciudad ingresada ya existe, Intente nuevamente!!");
            $("#ciudadM").focus();
        } else if (resp == 4) {
            alertify.error("Debe modificar la Ciudad!!");
            $("#ciudadM").focus();
        } else if (resp == 5) {
    ...
    for (i = 0; i < filas.length; i++) {
        filas[i].onclick = function() {
            ...
            $.ajax({
                type: "POST",
                dataType: 'html',
                url: "../servicios/BuscarModCiudad.php",
                data: "id=" + id,
            }).done(function(resp) { //se ejecuta cuando la solicitud Ajax ha concluido satisfactoriamente
                $("#ciudadM").val(resp);
                $("#ciudadSM").val(resp);
